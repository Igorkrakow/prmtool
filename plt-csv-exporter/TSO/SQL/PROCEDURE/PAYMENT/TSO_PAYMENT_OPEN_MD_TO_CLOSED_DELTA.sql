CREATE or replace PROCEDURE TXSTORE.TSO_PAYMENT_OPEN_MD_TO_CLOSED_DELTA(start_time TIMESTAMP, end_time TIMESTAMP)
    LANGUAGE SQL
BEGIN
    ...
    DECLARE V_CHECK_DRAW_FROM INTEGER;
    DECLARE V_LAST_DRAW_NUMBER INTEGER;
...
-- Closed wagers only
    ...
    AND TTOPT.CHECK_DRAW_FROM >= TXSTORE.LAST_CLOSED.DRAWNUMBER
...
--where LOTTERY_TX_HEADER_ID not in (select WAGER_TRANSACTION_ID from TXSTORE.TMP_TSO_REFUND)
     --MD WAGER
WHERE TTOPT.SD_OR_MD = 2 AND TTOPT.STATUS = 0;
...
FETCH MIGRATED_TX_CURSOR INTO V_START_DRAW_NUMBER,V_END_DRAW_NUMBER,V_LOTTERY_TX_HEADER_ID,
    V_GLOBAL_TRANS_ID,V_SERIAL,V_CDC,V_PLAYER_ID,V_CHECK_DRAW_FROM,V_LAST_DRAW_NUMBER;

SET V_COUNT_COMMIT = 1;
    WHILE (V_SQLCODE = 0)
        DO
            SET V_I = V_CHECK_DRAW_FROM;
            WHILE (V_I <= V_LAST_DRAW_NUMBER)
                DO
...
SELECT WAG.LOTTERY_TX_HEADER_ID,
       ...
       'i' FROM TXSTORE.LOTTERY_TX_HEADER WAG
        ...
        (WAG.SERIAL = VAL.SERIAL AND THW.PLAYER_ID = VAL.PLAYER_ID AND WAG.GLOBAL_TRANS_ID != VAL.GLOBAL_TRANS_ID AND V_I = VAL.START_DRAW_NUMBER)
WHERE  WAG.LOTTERY_TX_HEADER_ID = V_LOTTERY_TX_HEADER_ID;
SET V_I = V_I + 1;
end while;
UPDATE TXSTORE.TMP_TSO_OPEN_PAYMENT_TRANSACTION SET CHECK_DRAW_FROM = V_LAST_DRAW_NUMBER + 1,
STATUS = CASE WHEN V_LAST_DRAW_NUMBER >= V_END_DRAW_NUMBER THEN 1
ELSE 0 END,
END_UPDATE_DATE = end_time, START_UPDATE_DATE = start_time
...
FETCH MIGRATED_TX_CURSOR INTO V_START_DRAW_NUMBER,V_END_DRAW_NUMBER,
    V_LOTTERY_TX_HEADER_ID,V_GLOBAL_TRANS_ID,V_SERIAL,V_CDC,V_PLAYER_ID,V_CHECK_DRAW_FROM,V_LAST_DRAW_NUMBER;
IF (V_SQLCODE <> 0) THEN
