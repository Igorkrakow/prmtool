...
db2 "INSERT INTO TXSTORE.TMP_TSO_ROUND_SESSION(LOTTERY_TX_HEADER_ID, PLAYER_ID,
     ...
     --     Closed wagers only
         ...
         AND END_DRAW_NUMBER <= TXSTORE.LAST_CLOSED.DRAWNUMBER
             ...
             or (LHW.SERIAL=LHV.SERIAL
             AND THW.PLAYER_ID = LHV.PLAYER_ID
             and LHW.GLOBAL_TRANS_ID!=LHV.GLOBAL_TRANS_ID
             and LHV.START_DRAW_NUMBER between LHW.START_DRAW_NUMBER and LHW.END_DRAW_NUMBER)
...
log_with_timestamp "Starting KPI_TSO file_generation"
sh TSO/SH/KPI_TSO_EXTRACTION.sh "$startDate" "$endDate"
